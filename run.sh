...
#REQUIRED_PLUGINS="wp-super-cache,buddypress,bbpress"
REQUIRED_PLUGINS="buddypress,bbpress"
...
init() {
  ...
  PLUGINS="${REQUIRED_PLUGINS},${PLUGINS/%,},"
  THEMES="${THEMES/%,},"
...
install_buddypress() {
  h2 "Checking BuddyPress"
  wp plugin install --allow-root "buddypress" --version="$BP_VERSION" | tail -n 1 |& _colorize
}

install_bbpress() {
  h2 "Checking bbPress"
  wp plugin install --allow-root "bbpress" --version="$BB_VERSION" | tail -n 1 |& _colorize
}
...
check_database() {
  ...
  if [[ -n "$URL_REPLACE" ]]; then
    wp search-replace --skip-columns=guid "$BEFORE_URL" "$AFTER_URL" "wp_*" --allow-root \
    | grep 'replacement' \
    |& _colorize
  fi
}
...
main() {
  ...
  h2 "Checking BuddyPress and bbPress"
  install_buddypress
  install_bbpress
